<script setup lang="ts">
    ...
    onBeforeRender(({ delta }) => {
        if (!handlerInitialized && props.midi.isInitialized()) {
            props.midi.subscribeToEvent("on.three", "noteOn", (event) => {
                ...
                const material = new MeshStandardMaterial({ color: new Color().setHSL(event.midiNote / 128, 1, Math.min(Math.max(event.velocity, 16) - 16, 96) / 96), opacity: 0.8, transparent: true })
    ...
    const noiseMap = new NoiseMap('my-seed', 0.15, 16, 20, scene);
    const angle = ref(0)
    onBeforeRender(({ delta }) => {
